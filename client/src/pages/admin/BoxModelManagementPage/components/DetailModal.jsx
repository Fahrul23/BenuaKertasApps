...
const BoxModelDetailModal = ({ boxModel, onClose }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
          ...
          <div>
            <h2 className="text-base font-bold text-gray-900">Detail Box Model</h2>
            <p className="text-xs text-gray-500">{boxModel.code} — {boxModel.name}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex gap-4 items-start">
            ...
            <div className="flex-1 space-y-3">
              <InfoRow label="ID" value={`#${boxModel.id}`} />
              <div className="flex flex-col gap-0.5">
                <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Status</span>
                <span
                  className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold w-fit ${
                    boxModel.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                  }`}
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${boxModel.isActive ? 'bg-green-500' : 'bg-red-500'}`} />
                  {boxModel.isActive ? 'Aktif' : 'Tidak Aktif'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 bg-gray-50 rounded-xl space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <InfoRow label="Kode" value={boxModel.code} className="font-mono" />
              <InfoRow label="Nama" value={boxModel.name} />
            </div>
          ...
          <div className="p-4 bg-gray-50 rounded-xl space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3">
              <InfoRow label="Dibuat" value={formatDate(boxModel.createdAt)} />
              <InfoRow label="Diperbarui" value={formatDate(boxModel.updatedAt)} />
            </div>
          </div>
        </div>
...
export default BoxModelDetailModal;
